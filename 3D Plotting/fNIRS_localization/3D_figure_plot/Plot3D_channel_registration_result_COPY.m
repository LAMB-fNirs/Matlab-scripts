function Plot3D_channel_registration_result(intensity, MNIcoord, MNIcoordstd,brain_vol,minval,maxval)
% This function will plot the channel posistions on an 3D brain template
if isempty(brain_vol)
    load('MNI152_downsampled.mat'); % MNI152_nonlin_gm
...
intensity_1 = intensity_0*0.9;
intensity_2 = intensity_0*0.8;
intensity_3 = intensity_0*0.7;
intensity_4 = intensity_0*0.6;

intensity_optode=ones(8,1)*1;

%tmp = jet; % using ?hot? color scheme
tmp = jet;
mx = maxval;
mn = minval;
...
distanceThreshold_0 = MNIcoordstd*0.6;
distanceThreshold_1 = MNIcoordstd*0.75;
distanceThreshold_2 = MNIcoordstd*0.85;
distanceThreshold_3 = MNIcoordstd*0.95;
...
colormap jet;
colorbar ;
%caxis([min(intensity) max(intensity)]); %changed this
caxis([minval maxval]); %changed this
end
